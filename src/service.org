...
#+begin_src nim :noweb-ref volume-up
  ...
  play.pin = cast[int16](task["pin"].getNum())
  play.card_reader = cast[int8](task["speaker"].getNum())
  var pkt = play.encode_as(CommandType.VOLUME_UP, uid)
  discard task_buffer.add((uid: uid, sockaddr: dst, data: pkt))
  var saddr = dst
  <<ntop>>
  info "Prepare to send VOLUME_UP(speaker: ", play.card_reader, ", pin: ", cast[uint16](play.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref volume-down
  ...
  play.pin = cast[int16](task["pin"].getNum())
  play.card_reader = cast[int8](task["speaker"].getNum())
  var pkt = play.encode_as(CommandType.VOLUME_DOWN, uid)
  discard task_buffer.add((uid: uid, sockaddr: dst, data: pkt))
  var saddr = dst
  <<ntop>>
  info "Prepare to send VOLUME_DOWN(speaker: ", play.card_reader, ", pin: ", cast[uint16](play.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
