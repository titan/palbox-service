...
#+begin_src nim :noweb-ref charger-status-confirm
  ...
  await redis_client.multi()
  discard await redis_client.setEx(pluggedkey, 30, $param.charger_plugged)
...
#+begin_src nim :noweb-ref charger-config-confirm
  ...
  let
    charger = $param.board
    chargingkey = "charger-config." & $uid & "." & $charger & ".enable-charging"
    heatingkey = "charger-config." & $uid & "." & $charger & ".enable-heating"
  await redis_client.multi()
