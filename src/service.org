      ...
      if data != nil:
        case data.cmd_type:
          ...
          of CommandType.CONFIRM:
            info "Receive CONFIRM of ", data.parameter.cmd_type , " from ", address
            <<confirm>>
          ...
          if task == nil or task["expires-at"].getNum() < epochTime().toInt():
            ...
            if expires_at < now:
              ...
              if sending_time.contains(uid):
                if now - sending_time[uid] < 3: # Don't resend in 3 seconds
                  continue
              ...
              let
                ...
                tasklogs = if tasklog_len > 20: await redis_client.lrange(tasklog_key, tasklog_len - 20, tasklog_len - 1) else: await redis_client.lrange(tasklog_key, 0, -1)
              var found_log = false
