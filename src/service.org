...
#+begin_src nim :noweb-ref charger-status-confirm
  ...
  let
    plugginedkey = "charger-pluggined." & $uid
    chargingkey = "charger-charging." & $uid
    fullkey = "charger-full." & $uid
    heatingkey = "charger-heating." & $uid
    errnokey = "charger-errno." & $uid
  await redis_client.multi()
  discard await redis_client.setEx(plugginedkey, 30, $param.charger_pluggined)
  discard await redis_client.setEx(chargingkey, 30, $param.charger_charging)
  discard await redis_client.setEx(fullkey, 30, $param.charger_full)
  discard await redis_client.setEx(heatingkey, 30, $param.charger_heating)
  discard await redis_client.setEx(errnokey, 30, $param.errno)
  for i in 0..len(param.chargers) - 1:
    var skey = "charger-battery." & $uid & "." & $i
    discard await redis_client.setEx(skey, 30, $param.batteries[i])
  discard await redis_client.exec()
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
...
#+begin_src nim :noweb-ref charger-config-confirm
  ...
  let
    chargingkey = "charger-config." & $uid & ".enable-charging"
    heatingkey = "charger-config." & $uid & ".enable-heating"
  await redis_client.multi()
