...
#+begin_src nim :noweb-ref lock-off-confirm
  ...
  if param.pin != 0:
    discard await redis_client.setEx("lock-off-ack." & $uid & "." & $param.pin, 30, $param.timestamp)
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
...
#+begin_src nim :noweb-ref locks-off-confirm
  ...
  if param.pin != 0:
    discard await redis_client.setEx("locks-off-ack." & $uid & "." & $param.pin, 30, $param.timestamp)
...
#+begin_src nim :noweb-ref lock-data
  ...
  for i in 0..len(param.states) - 1:
    ...
    for j in 0..7:
      var skey = "lock-status." & $uid & "." & $param.board & "." & $((len(param.states) - 1 - i) * 8 + j + 1)
      if ((1 shl j) and state) != 0:
        discard await redis_client.setEx(skey, expires_in, "1")
      else:
        discard await redis_client.setEx(skey, expires_in, "0")
  <<lock-data-confirm>>
#+end_src
** 下行响应
...
#+begin_src nim :noweb-ref egg-lock-on-confirm
  ...
  discard await redis_client.setEx("egg-lock-on-ack." & $uid & "." & $param.pin, 30, $param.timestamp)
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
...
#+begin_src nim :noweb-ref egg-lock-off-confirm
  ...
  discard await redis_client.setEx("egg-lock-off-ack." & $uid & "." & $param.pin, 30, $param.timestamp)
  let taskstrs: seq[string] = await redis_client.lrange(taskkey, 0, -1)
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
...
#+begin_src nim :noweb-ref egg-play-confirm
  ...
  discard await redis_client.setEx("egg-play-ack." & $uid & "." & $param.pin, 30, $param.timestamp)
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
...
#+begin_src nim :noweb-ref egg-gpio-confirm
  ...
  discard await redis_client.setEx("egg-gpio-ack." & $uid & "." & $param.pin, 30, $param.timestamp)
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
...
#+begin_src nim :noweb-ref egg-volume-confirm
  ...
  discard await redis_client.setEx("egg-volume-ack." & $uid & "." & $param.pin, 30, $param.timestamp)
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
...
info "Confirm cmd type is egg-query, egg: ", param.board, ", cabin: ", param.lock
  await redis_client.multi()
  discard await redis_client.setEx("egg-query-ack." & $uid & "." & $param.pin, 30, $param.timestamp)
