...
#+begin_src nim :noweb-ref rfid-reader-data
  ...
  cmd.pin = cast[int16](task["pin"].getInt())
  cmd.board = cast[int8](task["board"].getInt())
  cmd.scope = case task["scope"].getStr():
                of "increment":
                  1
                of "decrement":
                  2
...
#+begin_src nim :noweb-ref rfid-reader-data-confirm
  ...
  let
    scope = case param.scope:
              ...
              of 2:
                ...
                "all"
    key = "rfid-reader-data." & $uid & "." & $param.board & "." & scope & "." & $param.offset & "." & $param.pin
  await redis_client.multi()
  ...
  if len(param.data) > 0:
    for i in 0..toInt(len(param.data)/6) - 1:
      let
        pos = i * 6
        val = param.data[pos].toHex() & param.data[pos + 1].toHex() & param.data[pos + 2].toHex() & param.data[pos + 3].toHex() & param.data[pos + 4].toHex() & param.data[pos + 5].toHex()
      discard await redis_client.sadd(key, val)
    discard await redis_client.expire(key, 300)
  discard await redis_client.exec()
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
    if task != nil and task["command"].getStr() == "rfid-reader-data" and task["board"].getInt() == param.board and task["scope"].getStr() == scope and task["offset"].getInt() == param.offset and cast[int16](task["pin"].getInt()) == param.pin:
      <<remove-task>>
#+end_src
* RFID-READER-INVENTORY-RESULT
