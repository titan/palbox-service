      ...
      for uid, pair in device_connections.pairs():
        ...
        if taskstr == redisNil:
          ...
          if task == nil or task["expires-at"].getNum() < now:
            ...
            if lrem_result == 0:
              warn "Cannot remove expired task ", taskstr, " from ", taskkey, " of ", box
            else:
              task["status"] = % "expired"
...
#+begin_src nim :noweb-ref service-ping
  ...
  ping.pin = cast[int16](task["pin"].getNum())
...
#+begin_src nim :noweb-ref pong
  ...
  if len(taskstrs) != 0:
    for taskstr in taskstrs:
      let task = try: parseJson(taskstr) except: nil
      if task != nil and task["command"].getStr() == "ping" and cast[int16](task["pin"].getNum()) == param.pin:
        <<remove-task>>
#+end_src
* CONFIRM
...
#+begin_src nim :noweb-ref config-gprs
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send CONFIG_GPRS(gprs-heart-rate: ", param.gprs_heart_rate, ", gprs-timeout: ", param.gprs_timeout, ", pin: ", cast[uint16](param.pin), ") to ", box, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
每一个从设备发送到 box service 的命令都携带了设备的 GPRS 状态信息。需
要将这些信息保存在缓存中，供数据挖掘程序使用。

#+begin_src nim :noweb-ref save-gprs-status
  let
  ...
  if gprs_shutdown != 0 or gprs_reset != 0:
