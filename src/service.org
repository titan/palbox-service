...
#+begin_src nim :noweb-ref charger-status-confirm
  ...
  await redis_client.multi()
  await redis_client.setk(pluggedkey, $param.charger_plugged)
  await redis_client.setk(chargingkey, $param.charger_charging)
  await redis_client.setk(fullkey, $param.charger_full)
  await redis_client.setk(heatingkey, $param.charger_heating)
  await redis_client.setk(errnokey, $param.errno)
  await redis_client.setk(temperaturekey, $param.temperature)
  for i in 0..len(param.batteries) - 1:
    var skey = "charger-battery." & $uid & "." & charger & "." & $i
    await redis_client.setk(skey, $param.batteries[i])
  discard await redis_client.exec()
  if param.pin != 0:
    for taskstr in taskstrs:
      let task = try: parseJson(taskstr) except: nil
