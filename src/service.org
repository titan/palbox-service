      ...
      for uid, pair in device_connections.pairs():
        ...
        if taskstr == redisNil:
          ...
          if task == nil or task["expires-at"].getNum() < now:
            if task["command"].getStr() == "lock-off" or task["command"].getStr() == "locks-off":
              # send a fan-on command to box device to check gprs network status
              let
                pin = generate_pin(1)
                jobkey = "tasks." & $uid
                logkey = "tasklog." & $uid & "." & getDateStr()
                job = %*{"command": "fan-on", "expires-at": epochTime().toInt() + 5, "pin": pin, "status": "queued", "occurred-at": getDateStr() & " " & getClockStr()}
              discard await redis_client.lpush(key = jobkey, value = $job)
...
#+begin_src nim :noweb-ref ping
  ...
  #info "PING uid", $uid
  let
    rssi_key = "rssi." & $uid & "." & getDateStr() & "." & $getLocalTime(getTime()).hour.format("02d") & "." & $getLocalTime(getTime()).minute.format("02d")
    ping_key = "ping." & $uid & "." & getDateStr() & "." & $getLocalTime(getTime()).hour.format("02d") & "." & $getLocalTime(getTime()).minute.format("02d")
    reset_key = "reset." & $uid & "." & getDateStr() & "." & $getLocalTime(getTime()).hour.format("02d") & "." & $getLocalTime(getTime()).minute.format("02d")
    reply_time = if param.reply_time == 0: 600 else: param.reply_time
    ttl = if param.ttl == 0: 255 else: param.ttl
    gprs_reset = param.gprs_reset
    gprs_shutdown = param.gprs_shutdown
  await redis_client.setk(rssi_key, $param.rssi)
  await redis_client.setk(ping_key, $reply_time & "," & $ttl)
  await redis_client.setk(reset_key, $gprs_shutdown & "," & $gprs_reset)
  <<pong>>
#+end_src
* PONG
...
#+begin_src nim :noweb-ref lock-off
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send LOCK_OFF(board: ", lock_off.board, ", lock: ", lock_off.lock , ", pin: ", cast[uint16](lock_off.pin) , ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref locks-off
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send LOCKS_OFF(board: ", locks_off.board, ", locks: ", locks_off.locks, ", pins: ", locks_off.pins.mapIt(cast[uint16](it)), ") to ", uid, " @ ", address
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref lock-status
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send LOCK_STATUS(board: ", lock_status.board, ", pin: ", cast[uint16](lock_status.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
#+begin_src nim :noweb-ref lock-status-confirm
  ...
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
    if task != nil and task["command"].getStr() == "lock-status" and cast[int8](task["board"].getNum()) == param.board and cast[int16](task["pin"].getNum()) == param.pin:
      <<remove-task>>
#+end_src
* LOCK-DETECT
** 下行命令
#+begin_src nim :noweb-ref lock-detect
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send LOCK_DETECT(board: ", lock_detect.board, ", pin: ", cast[uint16](lock_detect.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref lock-status-detect
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send LOCK_STATUS_DETECT(board: ", lock_status.board, ", pin: ", cast[uint16](lock_status.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref light-on
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send LIGHT_ON(pin: ", cast[uint16](light_on.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref light-off
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send LIGHT_Off(pin: ", cast[uint16](light_off.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref fan-on
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send FAN_ON(pin: ", cast[uint16](fan_on.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref fan-off
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send FAN_OFF(pin: ", cast[uint16](fan_off.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref ultraviolet-on
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send ULTRAVIOLET_ON(pin: ", cast[uint16](ultraviolet_on.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref ultraviolet-off
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send ULTRAVIOLET_OFF(pin: ", cast[uint16](ultraviolet_off.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref camera-on
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send CAMERA_ON(pin: ", cast[uint16](camera_on.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref camera-off
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send CAMERA_OFF(pin: ", cast[uint16](camera_off.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref charger-status
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send CHARGER_STATUS(pin: ", cast[uint16](charger_status.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
...
#+begin_src nim :noweb-ref play
  ...
  play.audio = cast[int16](task["audio"].getNum())
  play.card_reader = cast[int8](task["speaker"].getNum())
  var pkt = play.encode_as(CommandType.PLAY, uid)
  discard task_buffer.add((uid: uid, sockaddr: dst, data: pkt))
  var saddr = dst
  <<ntop>>
  info "Prepare to send PLAY(speaker: ", play.card_reader, ", audio: ", play.audio, ", pin: ", cast[uint16](play.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
#+begin_src nim :noweb-ref play-confirm
  ...
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
    if task != nil and task["command"].getStr() == "play" and task["speaker"].getNum() == param.card_reader and task["audio"].getNum() == param.audio and cast[int16](task["pin"].getNum()) == param.pin:
      <<remove-task>>
#+end_src
* VOLUME_UP
** 下行命令
#+begin_src nim :noweb-ref volume-up
  ...
  param.pin = cast[int16](task["pin"].getNum())
  param.card_reader = cast[int8](task["speaker"].getNum())
  var pkt = param.encode_as(CommandType.VOLUME_UP, uid)
  discard task_buffer.add((uid: uid, sockaddr: dst, data: pkt))
  var saddr = dst
  <<ntop>>
  info "Prepare to send VOLUME_UP(speaker: ", param.card_reader, ", pin: ", cast[uint16](param.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
#+begin_src nim :noweb-ref volume-up-confirm
  ...
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
    if task != nil and task["command"].getStr() == "volume-up" and task["speaker"].getNum() == param.card_reader and cast[int16](task["pin"].getNum()) == param.pin:
      <<remove-task>>
#+end_src
* VOLUME_DOWN
** 下行命令
#+begin_src nim :noweb-ref volume-down
  ...
  var saddr = dst
  <<ntop>>
  info "Prepare to send VOLUME_DOWN(speaker: ", param.card_reader, ", pin: ", cast[uint16](param.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
#+begin_src nim :noweb-ref volume-down-confirm
  ...
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
    if task != nil and task["command"].getStr() == "volume-down" and task["speaker"].getNum() == param.card_reader and cast[int16](task["pin"].getNum()) == param.pin:
      <<remove-task>>
#+end_src
* CONFIG_GPRS
** 下行命令
#+begin_src nim :noweb-ref config-gprs
  var param: ref Parameter = new(Parameter)
  param.pin = cast[int16](task["pin"].getNum())
  param.gprs_heart_rate = cast[int32](task["gprs-heart-rate"].getNum())
  param.gprs_timeout = cast[int32](task["gprs-timeout"].getNum())
  var pkt = param.encode_as(CommandType.CONFIG_GPRS, uid)
  discard task_buffer.add((uid: uid, sockaddr: dst, data: pkt))
  var saddr = dst
  <<ntop>>
  info "Prepare to send CONFIG_GPRS(gprs-heart-rate: ", param.gprs_heart_rate, ", gprs-timeout: ", param.gprs_timeout, ", pin: ", cast[uint16](param.pin), ") to ", uid, " @ ", address, ":", dst.sin_port
#+end_src
** 上行响应
#+begin_src nim :noweb-ref config-gprs-confirm
  info "Confirm cmd type is config-gprs-confirm"
  for taskstr in taskstrs:
    let task = try: parseJson(taskstr) except: nil
    if task != nil and task["command"].getStr() == "config-gprs" and cast[int16](task["pin"].getNum()) == param.pin:
      <<remove-task>>
#+end_src
* 数据封包
#+begin_src nim :exports code :noweb yes :mkdirp yes :tangle /dev/shm/box-service/src/box_servicepkg/box_packer.nim
  ...
  type
    CommandType* {.pure.} = enum
      UNKNOWN = 0, PING = 1, PONG = 2, CONFIRM = 3, LOCK_OFF = 4, LOCKS_OFF = 5, LOCK_STATUS = 6, LOCK_DETECT = 7, LOCK_STATUS_DETECT = 8, CHARGER_STATUS = 9, LIGHT_ON = 10, LIGHT_OFF = 11, FAN_ON = 12, FAN_OFF = 13, ULTRAVIOLET_ON = 14, ULTRAVIOLET_OFF = 15, CAMERA_ON = 16, CAMERA_OFF = 17, LOCK_DATA = 18, PLAY = 19, VOLUME_UP = 20, VOLUME_DOWN = 21, CONFIG_GPRS = 22
    UID* = array[0..11, byte]
